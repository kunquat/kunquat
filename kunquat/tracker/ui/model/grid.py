...
class Grid():
    ...
    def get_grid_lines(self, pat_num, col_num, start_ts, stop_ts):
        ...
        if grid_length > 0 and grid_lines:
            ...
            gp_start_ts = start_ts % grid_length

            line_index = 0
            line_pat_ts = (start_ts +
                    (grid_length + grid_lines[0][0] - gp_start_ts) % grid_length)
            for i, line in enumerate(grid_lines):
                line_ts, _ = line
                if line_ts >= gp_start_ts:
                    line_index = i
                    line_pat_ts = start_ts + line_ts - gp_start_ts
                    break
