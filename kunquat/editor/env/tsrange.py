...
class TsRange(QtGui.QWidget):
    ...
    @property
    def range(self):
        return [ts.Timestamp(self._begin.value()),
                ts.Timestamp(self._end.value())]

    @range.setter
    def range(self, r):
        if not r:
            r = [ts.Timestamp(0), ts.Timestamp(1)]
        assert self._allow_degen or r[0] != r[1]
        self._begin.blockSignals(True)
        self._end.blockSignals(True)
        try:
            self._begin.setValue(float(r[0]))
            self._end.setValue(float(r[1]))
        finally:
