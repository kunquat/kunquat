...
static bool Event_global_slide_volume_length_set(Event* event, int index, void* data);

static void* Event_global_slide_volume_length_get(Event* event, int index);
...
{
    assert(event != NULL);
    assert(event->type == EVENT_GLOBAL_SLIDE_VOLUME_LENGTH);
    assert(data != NULL);
    ...
    if (index == 0)
    {
        ...
        Reltime_copy(&slide_volume_length->length, length);
        return true;
    }
    return false;
}
...
static void* Event_global_slide_volume_length_get(Event* event, int index)
{
    ...
    if (index == 0)
    {
        return &slide_volume_length->length;
    }
    return NULL;
}
