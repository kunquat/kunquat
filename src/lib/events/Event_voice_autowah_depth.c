...
static bool Event_voice_autowah_depth_set(Event* event, int index, void* data);

static void* Event_voice_autowah_depth_get(Event* event, int index);
...
{
    assert(event != NULL);
    assert(event->type == EVENT_VOICE_AUTOWAH_DEPTH);
    assert(data != NULL);
