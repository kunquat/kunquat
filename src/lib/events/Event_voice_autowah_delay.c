...
static bool Event_voice_autowah_delay_set(Event* event, int index, void* data);

static void* Event_voice_autowah_delay_get(Event* event, int index);
...
{
    assert(event != NULL);
    assert(event->type == EVENT_VOICE_AUTOWAH_DELAY);
    assert(data != NULL);
    ...
    Reltime_copy(&autowah_delay->delay, delay);
    return true;
}
