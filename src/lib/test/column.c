...
START_TEST (new)
{
    ...
    del_Column(col);
    col = new_Column(kqt_Reltime_init(KQT_RELTIME_AUTO));
    if (col == NULL)
    {
        fprintf(stderr, "new_Column() returned NULL -- out of memory?\n");
        abort();
    ...
    fail_unless(Column_iter_get(citer, kqt_Reltime_init(KQT_RELTIME_AUTO)) == NULL,
            "new_Column() created a non-empty Column.");
...
START_TEST (ins)
{
    ...
    {
        Event* event = new_Event_voice_note_on(kqt_Reltime_set(KQT_RELTIME_AUTO, i, 0));
        if (event == NULL)
        {
        ...
        Event* ret = Column_iter_get(citer, kqt_Reltime_set(KQT_RELTIME_AUTO, i, 0));
        fail_if(ret == NULL,
    ...
    Event* event = new_Event_voice_note_on(kqt_Reltime_set(KQT_RELTIME_AUTO, 2, 0));
    if (event == NULL)
    {
        fprintf(stderr, "new_Event() returned NULL -- out of memory?\n");
        abort();
    ...
    Event* ret = Column_iter_get(citer, kqt_Reltime_set(KQT_RELTIME_AUTO, 2, 0));
    fail_if(ret == NULL,
...
START_TEST (col_remove)
{
    ...
    events[0] = new_Event_voice_note_on(kqt_Reltime_set(KQT_RELTIME_AUTO, 0, 0));
    if (events[0] == NULL)
    {
        fprintf(stderr, "new_Event() returned NULL -- out of memory?\n");
        abort();
    ...
    fail_unless(Column_iter_get(citer, kqt_Reltime_set(KQT_RELTIME_AUTO, 0, 0)) == NULL,
            "Column_get() returned an Event from an empty Column.");
    for (int64_t i = 0; i < 7; ++i)
    {
        events[i] = new_Event_voice_note_on(kqt_Reltime_set(KQT_RELTIME_AUTO, i, 0));
        if (events[i] == NULL)
        {
            fprintf(stderr, "new_Event() returned NULL -- out of memory?\n");
            abort();
    ...
    Event* ret = Column_iter_get(citer, kqt_Reltime_init(KQT_RELTIME_AUTO));
    ...
    ret = Column_iter_get(citer, kqt_Reltime_init(KQT_RELTIME_AUTO));
    fail_unless(ret == events[1],
            "Column_get() returned Event %p instead of %p.", ret, events[1]);
    for (int64_t i = 2; i < 7; ++i)
    {
    ...
    for (int64_t i = 0; i < 7; ++i)
    {
        events[i] = new_Event_voice_note_on(kqt_Reltime_set(KQT_RELTIME_AUTO, 1, 0));
        if (events[i] == NULL)
        {
            fprintf(stderr, "new_Event() returned NULL -- out of memory?\n");
            abort();
        ...
        Event* ret = Column_iter_get(citer, kqt_Reltime_set(KQT_RELTIME_AUTO, 1, 0));
        fail_if(ret == NULL,
                "Couldn't find the Event inserted at beat %lld.", (long long)i);
...
START_TEST (clear)
{
    ...
    for (int64_t i = 0; i < 7; ++i)
    {
        ...
        fail_if(Column_iter_get(citer, kqt_Reltime_init(KQT_RELTIME_AUTO)) == NULL,
...
START_TEST (length)
{
    ...
    kqt_Reltime* len = kqt_Reltime_set(KQT_RELTIME_AUTO, 1, 2);
    Column_set_length(col, len);
    kqt_Reltime* ret = Column_length(col);
    fail_if(ret == len,
            "Column_set_length() stored the passed kqt_Reltime instead of copying.");
    fail_unless(kqt_Reltime_cmp(ret, len) == 0,
...
{
    Column_set_length(NULL, kqt_Reltime_init(KQT_RELTIME_AUTO));
}
END_TEST
