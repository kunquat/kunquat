...
START_TEST (new)
{
    ...
    table = new_Scale(DBL_MIN, &octave_ratio);
    if (table == NULL)
    {
    ...
    fail_unless(table->ref_pitch == DBL_MIN,
            "new_Scale() didn't set reference pitch correctly.");
    fail_unless(Real_cmp(&octave_ratio, &(table->octave_ratio)) == 0,
            "new_Scale() didn't set octave ratio correctly.");
    Real_init_as_frac(&octave_ratio, 1, 1);
    fail_unless(Real_cmp(&octave_ratio, &(table->octave_ratio)) != 0,
            "new_Scale() incorrectly copied the reference of octave ratio.");
    Real_init_as_frac(&octave_ratio, 2, 1);
    fail_unless(table->note_count == 0,
            "new_Scale() didn't create an empty table.");
    fail_unless(table->ref_note == table->ref_note_retuned,
            "new_Scale() didn't set retuned reference note correctly.");
    for (i = 1; i < KQT_SCALE_OCTAVES; ++i)
    {
        ...
        fail_unless(Real_cmp(&quotient, &octave_ratio) == 0,
                "new_Scale() didn't create oct_factors[] correctly.");
    }
    for (i = 0; i < KQT_SCALE_NOTE_MODS; ++i)
    {
        ...
        fail_unless(Real_cmp(&(table->note_mods[i].ratio), &one) == 0,
                "new_Scale() didn't initialise note modifiers correctly.");
    }
    for (i = 0; i < KQT_SCALE_NOTES; ++i)
    {
        ...
        fail_unless(Real_cmp(&(table->notes[i].ratio), &one) == 0,
                "new_Scale() didn't initialise notes correctly.");
    ...
    table = new_Scale(DBL_MAX, &octave_ratio);
    if (table == NULL)
    {
        fprintf(stderr, "new_Scale() returned NULL -- Out of memory?\n");
        abort();
    ...
    table = new_Scale(528, &octave_ratio);
    if (table == NULL)
    {
        fprintf(stderr, "new_Scale() returned NULL -- Out of memory?\n");
        abort();
...
START_TEST (new_break2)
{
    Real octave_ratio;
    Real_init(&octave_ratio);
    new_Scale(-DBL_MIN, &octave_ratio);
}
END_TEST
...
START_TEST (new_break3)
{
    Real octave_ratio;
    Real_init(&octave_ratio);
    new_Scale(-DBL_MAX, &octave_ratio);
}
END_TEST
...
START_TEST (set_ref_pitch)
{
    ...
    Scale_set_ref_pitch(table, DBL_MIN);
    fail_unless(table->ref_pitch == DBL_MIN,
            "Scale_set_ref_pitch() didn't set reference pitch correctly.");
    Scale_set_ref_pitch(table, DBL_MAX);
    fail_unless(table->ref_pitch == DBL_MAX,
            "Scale_set_ref_pitch() didn't set reference pitch correctly.");
    del_Scale(table);
}
...
START_TEST (get_ref_pitch)
{
    ...
    fail_unless(Scale_get_ref_pitch(table) == 528,
            "Scale_get_ref_pitch() didn't return the correct reference pitch.");
    Scale_set_ref_pitch(table, DBL_MIN);
    fail_unless(Scale_get_ref_pitch(table) == DBL_MIN,
            "Scale_get_ref_pitch() didn't return the correct reference pitch.");
    Scale_set_ref_pitch(table, DBL_MAX);
    fail_unless(Scale_get_ref_pitch(table) == DBL_MAX,
            "Scale_get_ref_pitch() didn't return the correct reference pitch.");
    del_Scale(table);
}
END_TEST
...
START_TEST (set_octave_ratio)
{
    ...
    Real_init_as_frac(&octave_ratio, 2, 1);
    Scale_set_octave_ratio(table, &octave_ratio);
    fail_unless(Real_cmp(&octave_ratio, &(table->octave_ratio)) == 0,
            "Scale_set_octave_ratio() didn't set octave ratio correctly.");
    for (i = 1; i < KQT_SCALE_OCTAVES; ++i)
    {
        Real quotient;
        Real_div(&quotient, &(table->oct_factors[i]), &(table->oct_factors[i - 1]));
        fail_unless(Real_cmp(&quotient, &octave_ratio) == 0,
                "Scale_set_octave_ratio() didn't create oct_factors[] correctly.");
    }
    Real_init_as_frac(&octave_ratio, 1, 1);
    fail_unless(Real_cmp(&octave_ratio, &(table->octave_ratio)) != 0,
            "Scale_set_octave_ratio() incorrectly copied the reference of octave ratio.");
...
START_TEST (set_octave_ratio_break1)
{
    Real octave_ratio;
    Real_init(&octave_ratio);
    Scale_set_octave_ratio(NULL, &octave_ratio);
}
END_TEST
...
START_TEST (set_octave_ratio_break2)
{
    ...
    Scale_set_octave_ratio(table, NULL);
...
START_TEST (get_octave_ratio)
{
    ...
    fail_unless(Real_cmp(&octave_ratio, Scale_get_octave_ratio(table)) == 0,
            "Scale_get_octave_ratio() didn't return the correct octave ratio.");
...
START_TEST (set_note)
{
    ...
    Real_init_as_frac(&octave_ratio, 2, 1);
    table = new_Scale(440, &octave_ratio);
    if (table == NULL)
    {
        fprintf(stderr, "new_Scale() returned NULL -- Out of memory?\n");
        abort();
    ...
    actual_index = Scale_set_note(table, 0, &note_ratio);
    fail_unless(actual_index == 0,
            "Scale_set_note() didn't return the correct index.");
    fail_unless(Real_cmp(&(table->notes[0].ratio), &note_ratio) == 0,
            "Scale_set_note() didn't set the note ratio correctly.");
    fail_unless(Real_cmp(&(table->notes[0].ratio_retuned), &note_ratio) == 0,
            "Scale_set_note() didn't set the retuned note ratio correctly.");
    Real_init_as_frac(&note_ratio, 1, 1);
    fail_unless(Real_cmp(&(table->notes[0].ratio), &note_ratio) != 0,
            "Scale_set_note() incorrectly copied the reference of the note ratio.");
    fail_unless(Real_cmp(&(table->notes[0].ratio_retuned), &note_ratio) != 0,
            "Scale_set_note() incorrectly copied the reference of the retuned note ratio.");
    fail_unless(table->note_count == 1,
            "Scale_set_note() didn't increment the note count correctly.");

    for (i = 1; i < KQT_SCALE_NOTES; ++i)
    {
        Real_init_as_frac(&note_ratio, i, i + 1);
        actual_index = Scale_set_note(table, KQT_SCALE_NOTES - 1, &note_ratio);
        ...
        if (i < KQT_SCALE_NOTES - 1)
        {
            fail_if(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio), &note_ratio) == 0,
                    "Scale_set_note() incorrectly set the note ratio at the end of table.");
            fail_if(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio_retuned), &note_ratio) == 0,
                    "Scale_set_note() incorrectly set the retuned note ratio at the end of table.");
        }
    }
    for (i = 1; i < KQT_SCALE_NOTES; ++i)
    {
...
START_TEST (set_note_break_table)
{
    ...
    Scale_set_note(NULL, 0, &note_ratio);
}
END_TEST
...
START_TEST (set_note_break_index1)
{
    ...
    Scale_set_note(table, -1, &note_ratio);
...
START_TEST (set_note_break_index2)
{
    ...
    Scale_set_note(table, INT_MIN, &note_ratio);
...
START_TEST (set_note_break_index3)
{
    ...
    Scale_set_note(table, KQT_SCALE_NOTES, &note_ratio);
...
START_TEST (set_note_break_index4)
{
    ...
    Scale_set_note(table, INT_MAX, &note_ratio);
...
START_TEST (set_note_break_ratio2)
{
    ...
    Scale_set_note(table, 0, &note_ratio);
...
START_TEST (ins_note)
{
    ...
    actual_index = Scale_ins_note(table, 0, &note_ratio);
    fail_unless(actual_index == 0,
            "Scale_ins_note() didn't return the correct index.");
    fail_unless(Real_cmp(&(table->notes[0].ratio), &note_ratio) == 0,
            "Scale_ins_note() didn't set the note ratio correctly.");
    fail_unless(Real_cmp(&(table->notes[0].ratio_retuned), &note_ratio) == 0,
            "Scale_ins_note() didn't set the retuned note ratio correctly.");
    Real_init_as_frac(&note_ratio, 1, 1);
    fail_unless(Real_cmp(&(table->notes[0].ratio), &note_ratio) != 0,
            "Scale_ins_note() incorrectly copied the reference of the note ratio.");
    fail_unless(Real_cmp(&(table->notes[0].ratio_retuned), &note_ratio) != 0,
            "Scale_ins_note() incorrectly copied the reference of the retuned note ratio.");
    fail_unless(table->note_count == 1,
            "Scale_ins_note() didn't increment the note count correctly.");
...
/*      fprintf(stderr, "C should be at index %d, table->note_count = %d\n", i + 1, table->note_count); */
        if (i < KQT_SCALE_NOTES - 2)
        {
            ...
            fail_unless(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio), &note_ratio) == 0,
                    "Scale_ins_note() put garbage after the inserted notes.");
        ...
        else
        {
            Real_init_as_frac(&note_ratio, 6, 5);
            fail_unless(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio), &note_ratio) == 0,
                    "Scale_ins_note() didn't shift notes forward correctly.");
            fail_unless(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio_retuned), &note_ratio) == 0,
                    "Scale_ins_note() didn't shift notes forward correctly.");
        }
    }
    ...
    actual_index = Scale_ins_note(table, 1, &note_ratio);
    fail_unless(actual_index == 1,
            "Scale_ins_note() didn't return the correct index.");
    Real_init_as_frac(&note_ratio, 1, 2);
    fail_unless(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio), &note_ratio) == 0,
            "Scale_ins_note() didn't replace the last note correctly.");
    fail_unless(Real_cmp(&(table->notes[KQT_SCALE_NOTES - 1].ratio_retuned), &note_ratio) == 0,
            "Scale_ins_note() didn't replace the last note correctly.");
...
START_TEST (ins_note_break_table)
{
    Real note_ratio;
    Real_init(&note_ratio);
    Scale_ins_note(NULL, 0, &note_ratio);
}
END_TEST
...
START_TEST (ins_note_break_index1)
{
    ...
    Scale_ins_note(table, -1, &note_ratio);
...
START_TEST (ins_note_break_index2)
{
    ...
    Scale_ins_note(table, INT_MIN, &note_ratio);
...
START_TEST (ins_note_break_index3)
{
    ...
    Scale_ins_note(table, KQT_SCALE_NOTES, &note_ratio);
...
START_TEST (ins_note_break_index4)
{
    ...
    Scale_ins_note(table, INT_MAX, &note_ratio);
...
START_TEST (ins_note_break_ratio2)
{
    ...
    Scale_ins_note(table, 0, &note_ratio);
...
START_TEST (del_note)
{
    ...
    Scale_del_note(table, 0);
    fail_unless(table->note_count == 0,
            "Scale_del_note() modified the note count of an empty table.");
    Scale_del_note(table, KQT_SCALE_NOTES - 1);
    fail_unless(table->note_count == 0,
            "Scale_del_note() modified the note count of an empty table.");
    ...
    {
        Real_init_as_frac(&note_ratio, i + 1, i + 2);
        Scale_set_note(table, i, &note_ratio);
    }
    assert(table->note_count == KQT_SCALE_NOTES);
    old_count = table->note_count;
    Scale_del_note(table, KQT_SCALE_NOTES - 1);
    fail_unless(table->note_count == old_count - 1,
            "Scale_del_note() didn't decrease the note count correctly.");

    old_count = table->note_count;
    Scale_del_note(table, 8);
    fail_unless(table->note_count == old_count - 1,
            "Scale_del_note() didn't decrease the note count correctly.");
    for (i = 0; i < KQT_SCALE_NOTES - 2; ++i)
    {
        ...
        fail_unless(Real_cmp(&(table->notes[i].ratio), &note_ratio) == 0,
                "Scale_del_note() didn't shift subsequent notes backward correctly.");
        fail_unless(Real_cmp(&(table->notes[i].ratio_retuned), &note_ratio) == 0,
                "Scale_del_note() didn't shift subsequent notes backward correctly.");
    }
...
START_TEST (del_note_break_index3)
{
    ...
    Scale_del_note(table, KQT_SCALE_NOTES);
...
START_TEST (move_note)
{
    ...
    actual_index = Scale_move_note(table, 0, KQT_SCALE_NOTES - 1);
    fail_unless(actual_index == KQT_SCALE_NOTES - 1,
            "Scale_move_note() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTES; ++i)
    {
        if (i == KQT_SCALE_NOTES - 1)
        {
            Real_init_as_frac(&note_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->notes[i].ratio), &note_ratio) == 0,
                    "Scale_move_note() didn't modify the note at new_index correctly.");
    ...
    Scale_del_note(table, 0);
    actual_index = Scale_move_note(table, KQT_SCALE_NOTES - 2, 0);
    fail_unless(actual_index == 0,
            "Scale_move_note() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTES - 1; ++i)
    {
    ...
    actual_index = Scale_move_note(table, KQT_SCALE_NOTES - 1, 0); /* src is empty */
    fail_unless(actual_index == KQT_SCALE_NOTES - 1,
            "Scale_move_note() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTES - 1; ++i)
    {
        if (i == 0)
        {
            Real_init_as_frac(&note_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->notes[i].ratio), &note_ratio) == 0,
                    "Scale_move_note() incorrectly modified the table.");
            Real_init_as_frac(&note_ratio, 2, 1);
            fail_unless(Real_cmp(&(table->notes[i].ratio_retuned), &note_ratio) == 0,
                    "Scale_move_note() incorrectly modified the table.");
        }
    ...
    actual_index = Scale_move_note(table, 0, KQT_SCALE_NOTES - 1); /* dest is empty */
    ...
    for (i = 0; i < KQT_SCALE_NOTES - 1; ++i)
    {
        if (i == KQT_SCALE_NOTES - 2)
        {
            Real_init_as_frac(&note_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->notes[i].ratio), &note_ratio) == 0,
                    "Scale_move_note() didn't handle empty note at new_index correctly.");
            Real_init_as_frac(&note_ratio, 2, 1);
            fail_unless(Real_cmp(&(table->notes[i].ratio_retuned), &note_ratio) == 0,
                    "Scale_move_note() didn't handle empty note at new_index correctly.");
        }
...
START_TEST (move_note_break_index2)
{
    ...
    Scale_move_note(table, INT_MIN, 0);
...
START_TEST (move_note_break_index3)
{
    ...
    Scale_move_note(table, KQT_SCALE_NOTES, 0);
...
START_TEST (move_note_break_index4)
{
    ...
    Scale_move_note(table, INT_MAX, 0);
...
START_TEST (move_note_break_new_index2)
{
    ...
    Scale_move_note(table, 0, INT_MIN);
...
START_TEST (move_note_break_new_index3)
{
    ...
    Scale_move_note(table, 0, KQT_SCALE_NOTES);
...
START_TEST (move_note_break_new_index4)
{
    ...
    Scale_move_note(table, 0, INT_MAX);
...
START_TEST (get_note_ratio)
{
    ...
    fail_unless(Scale_get_note_ratio(table, 0) == NULL,
            "Scale_get_note_ratio() didn't return NULL for non-existent note.");
    ...
    {
        Real_init_as_frac(&note_ratio, i + 1, i + 2);
        fail_unless(Real_cmp(Scale_get_note_ratio(table, i), &note_ratio) == 0,
                "Scale_get_note_ratio() didn't return the correct ratio.");
    }
...
START_TEST (get_note_ratio_break_index2)
{
    ...
    Scale_get_note_ratio(table, INT_MIN);
...
START_TEST (get_note_ratio_break_index3)
{
    ...
    Scale_get_note_ratio(table, KQT_SCALE_NOTES);
...
START_TEST (get_note_ratio_break_index4)
{
    ...
    Scale_get_note_ratio(table, INT_MAX);
...
START_TEST (set_note_mod)
{
    ...
    actual_index = Scale_set_note_mod(table, 0, &mod_ratio);
    fail_unless(actual_index == 0,
            "Scale_set_note_mod() didn't return the correct index.");
    fail_unless(Real_cmp(&(table->note_mods[0].ratio), &mod_ratio) == 0,
            "Scale_set_note_mod() didn't set the modifier ratio correctly.");
    Real_init_as_frac(&mod_ratio, 1, 1);
    fail_unless(Real_cmp(&(table->note_mods[0].ratio), &mod_ratio) != 0,
            "Scale_set_note_mod() incorrectly copied the reference of the modifier ratio.");

    for (i = 1; i < KQT_SCALE_NOTE_MODS; ++i)
    {
        Real_init_as_frac(&mod_ratio, i, i + 1);
        actual_index = Scale_set_note_mod(table, KQT_SCALE_NOTE_MODS - 1, &mod_ratio);
        fail_unless(actual_index == i,
                "Scale_set_note_mod() didn't return the correct index.");
        if (i < KQT_SCALE_NOTE_MODS - 1)
        {
            fail_if(Real_cmp(&(table->note_mods[KQT_SCALE_NOTE_MODS - 1].ratio), &mod_ratio) == 0,
                    "Scale_set_note_mod() incorrectly set the modifier ratio at the end of table.");
        }
    }
    for (i = 1; i < KQT_SCALE_NOTE_MODS; ++i)
    {
...
START_TEST (set_note_mod_break_table)
{
    Real mod_ratio;
    Real_init(&mod_ratio);
    Scale_set_note_mod(NULL, 0, &mod_ratio);
}
END_TEST
...
START_TEST (set_note_mod_break_index1)
{
    ...
    Scale_set_note_mod(table, -1, &mod_ratio);
...
START_TEST (set_note_mod_break_index2)
{
    ...
    Scale_set_note_mod(table, INT_MIN, &mod_ratio);
...
START_TEST (set_note_mod_break_index3)
{
    ...
    Scale_set_note_mod(table, KQT_SCALE_NOTE_MODS, &mod_ratio);
...
START_TEST (set_note_mod_break_index4)
{
    ...
    Scale_set_note_mod(table, INT_MAX, &mod_ratio);
...
START_TEST (set_note_mod_break_ratio1)
{
    ...
    Scale_set_note_mod(table, 0, NULL);
...
START_TEST (set_note_mod_break_ratio2)
{
    ...
    Scale_set_note_mod(table, 0, &mod_ratio);
...
START_TEST (ins_note_mod)
{
    ...
    actual_index = Scale_ins_note_mod(table, 0, &mod_ratio);
    fail_unless(actual_index == 0,
            "Scale_ins_note_mod() didn't return the correct index.");
    fail_unless(Real_cmp(&(table->note_mods[0].ratio), &mod_ratio) == 0,
            "Scale_ins_note_mod() didn't set the modifier ratio correctly.");
    Real_init_as_frac(&mod_ratio, 1, 1);
    fail_unless(Real_cmp(&(table->note_mods[0].ratio), &mod_ratio) != 0,
            "Scale_ins_note_mod() incorrectly copied the reference of the modifier ratio.");
...
/*      fprintf(stderr, "# should be at index %d\n", i + 1); */
        if (i < KQT_SCALE_NOTE_MODS - 2)
        {
            ...
            fail_unless(Real_cmp(&(table->note_mods[KQT_SCALE_NOTE_MODS - 1].ratio), &mod_ratio) == 0,
                    "Scale_ins_note_mod() put garbage after the inserted modifiers.");
        }
        else
        {
            Real_init_as_frac(&mod_ratio, 16, 15);
            fail_unless(Real_cmp(&(table->note_mods[KQT_SCALE_NOTE_MODS - 1].ratio), &mod_ratio) == 0,
                    "Scale_ins_note_mod() didn't shift modifiers forward correctly.");
        }
    }
    for (i = 0; i < KQT_SCALE_NOTE_MODS - 1; ++i)
    {
    ...
    actual_index = Scale_ins_note_mod(table, 1, &mod_ratio);
    fail_unless(actual_index == 1,
            "Scale_ins_note_mod() didn't return the correct index.");
    Real_init_as_frac(&mod_ratio, 1, 2);
    fail_unless(Real_cmp(&(table->note_mods[KQT_SCALE_NOTE_MODS - 1].ratio), &mod_ratio) == 0,
            "Scale_ins_note_mod() didn't replace the last modifier correctly.");
...
START_TEST (ins_note_mod_break_table)
{
    Real mod_ratio;
    Real_init(&mod_ratio);
    Scale_ins_note_mod(NULL, 0, &mod_ratio);
}
END_TEST
...
START_TEST (ins_note_mod_break_index1)
{
    ...
    Scale_ins_note_mod(table, -1, &mod_ratio);
...
START_TEST (ins_note_mod_break_index2)
{
    ...
    Scale_ins_note_mod(table, INT_MIN, &mod_ratio);
...
START_TEST (ins_note_mod_break_index3)
{
    ...
    Scale_ins_note_mod(table, KQT_SCALE_NOTE_MODS, &mod_ratio);
...
START_TEST (ins_note_mod_break_index4)
{
    ...
    Scale_ins_note_mod(table, INT_MAX, &mod_ratio);
...
START_TEST (ins_note_mod_break_ratio1)
{
    ...
    Scale_ins_note_mod(table, 0, NULL);
...
START_TEST (ins_note_mod_break_ratio2)
{
    ...
    Scale_ins_note_mod(table, 0, &mod_ratio);
...
START_TEST (del_note_mod)
{
    ...
    Scale_del_note_mod(table, 0);

    for (i = 0; i < KQT_SCALE_NOTE_MODS; ++i)
    {
        Real_init_as_frac(&mod_ratio, i + 1, i + 2);
        Scale_set_note_mod(table, i, &mod_ratio);
    }
    Scale_del_note_mod(table, KQT_SCALE_NOTE_MODS - 1);

    Scale_del_note_mod(table, 3);
    for (i = 0; i < KQT_SCALE_NOTE_MODS - 2; ++i)
    {
        ...
        fail_unless(Real_cmp(&(table->note_mods[i].ratio), &mod_ratio) == 0,
                "Scale_del_note_mod() didn't shift subsequent modifiers backward correctly.");
    }
...
START_TEST (del_note_mod_break_index2)
{
    ...
    Scale_del_note_mod(table, INT_MIN);
...
START_TEST (del_note_mod_break_index3)
{
    ...
    Scale_del_note_mod(table, KQT_SCALE_NOTE_MODS);
...
START_TEST (del_note_mod_break_index4)
{
    ...
    Scale_del_note_mod(table, INT_MAX);
...
START_TEST (move_note_mod)
{
    ...
    actual_index = Scale_move_note_mod(table, 0, KQT_SCALE_NOTE_MODS - 1);
    fail_unless(actual_index == KQT_SCALE_NOTE_MODS - 1,
            "Scale_move_note_mod() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTE_MODS; ++i)
    {
        if (i == KQT_SCALE_NOTE_MODS - 1)
        {
            Real_init_as_frac(&mod_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->note_mods[i].ratio), &mod_ratio) == 0,
                    "Scale_move_note_mod() didn't modify the modifier at new_index correctly.");
        }
        else
        {
            Real_init_as_frac(&mod_ratio, i + 2, i + 3);
    ...
    Scale_del_note_mod(table, 0);
    actual_index = Scale_move_note_mod(table, KQT_SCALE_NOTE_MODS - 2, 0);
    fail_unless(actual_index == 0,
            "Scale_move_note_mod() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTE_MODS - 1; ++i)
    {
    ...
    actual_index = Scale_move_note_mod(table, KQT_SCALE_NOTE_MODS - 1, 0); /* src is empty */
    fail_unless(actual_index == KQT_SCALE_NOTE_MODS - 1,
            "Scale_move_note_mod() didn't return the correct index.");
    for (i = 0; i < KQT_SCALE_NOTE_MODS - 1; ++i)
    {
        if (i == 0)
        {
            Real_init_as_frac(&mod_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->note_mods[i].ratio), &mod_ratio) == 0,
                    "Scale_move_note_mod() incorrectly modified the table.");
        }
        else
        {
            Real_init_as_frac(&mod_ratio, i + 2, i + 3);
    ...
    actual_index = Scale_move_note_mod(table, 0, KQT_SCALE_NOTE_MODS - 1); /* dest is empty */
    ...
    for (i = 0; i < KQT_SCALE_NOTE_MODS - 1; ++i)
    {
        if (i == KQT_SCALE_NOTE_MODS - 2)
        {
            Real_init_as_frac(&mod_ratio, 1, 2);
            fail_unless(Real_cmp(&(table->note_mods[i].ratio), &mod_ratio) == 0,
                    "Scale_move_note_mod() didn't handle empty modifier at new_index correctly.");
        }
        else
        {
...
START_TEST (move_note_mod_break_index2)
{
    ...
    Scale_move_note_mod(table, INT_MIN, 0);
...
START_TEST (move_note_mod_break_index3)
{
    ...
    Scale_move_note_mod(table, KQT_SCALE_NOTE_MODS, 0);
...
START_TEST (move_note_mod_break_index4)
{
    ...
    Scale_move_note_mod(table, INT_MAX, 0);
...
START_TEST (move_note_mod_break_new_index2)
{
    ...
    Scale_move_note_mod(table, 0, INT_MIN);
...
START_TEST (move_note_mod_break_new_index3)
{
    ...
    Scale_move_note_mod(table, 0, KQT_SCALE_NOTE_MODS);
...
START_TEST (move_note_mod_break_new_index4)
{
    ...
    Scale_move_note_mod(table, 0, INT_MAX);
...
START_TEST (get_note_mod_ratio)
{
    ...
    fail_unless(Scale_get_note_mod_ratio(table, 0) == NULL,
            "Scale_get_note_mod_ratio() didn't return NULL for non-existent modifier.");
    ...
    {
        Real_init_as_frac(&mod_ratio, i + 1, i + 2);
        fail_unless(Real_cmp(Scale_get_note_mod_ratio(table, i), &mod_ratio) == 0,
                "Scale_get_note_mod_ratio() didn't return the correct ratio.");
    }
...
{
    Scale_get_note_mod_ratio(NULL, 0);
}
END_TEST
...
START_TEST (get_note_mod_ratio_break_index1)
{
    ...
    Scale_get_note_mod_ratio(table, -1);
...
START_TEST (get_note_mod_ratio_break_index2)
{
    ...
    Scale_get_note_mod_ratio(table, INT_MIN);
...
START_TEST (get_note_mod_ratio_break_index3)
{
    ...
    Scale_get_note_mod_ratio(table, KQT_SCALE_NOTE_MODS);
...
START_TEST (get_note_mod_ratio_break_index4)
{
    ...
    Scale_get_note_mod_ratio(table, INT_MAX);
...
START_TEST (get_pitch)
{
    ...
    fail_unless(Scale_get_pitch(table, 0, -1, KQT_SCALE_MIDDLE_OCTAVE) < 0,
            "Scale_get_pitch() didn't return a negative value for non-existent note.");
    ...
    Scale_set_note_mod(table, 0, &note_ratio);
    Real_init_as_frac(&note_ratio, 15, 16);
    Scale_set_note_mod(table, 1, &note_ratio);
    ...
    for (i = 0; i < KQT_SCALE_NOTES; ++i)
    {
        ...
        {
            fail_if(Scale_get_pitch(table, i, -1, j)
                    - Real_mul_float(&wrong_ratio, base_factor) < 0.00000001,
                    "Scale_get_pitch() didn't use the retuned ratios.");
            fail_unless(Scale_get_pitch(table, i, -1, j)
                    - Real_mul_float(&note_ratio, base_factor) < 0.00000001,
                    "Scale_get_pitch() didn't calculate the pitch accurately enough.");
            fail_unless(Scale_get_pitch(table, i, -1, j)
                    - Scale_get_pitch(table, i, INT_MIN, j) < 0.00000001,
                    "Scale_get_pitch() returned different results for mod = -1 and mod = INT_MIN.");
    ...
    for (i = 0; i < KQT_SCALE_NOTES; ++i)
    {
        ...
        for (j = KQT_SCALE_MIDDLE_OCTAVE; j >= KQT_SCALE_OCTAVE_FIRST; --j)
        {
            fail_if(Scale_get_pitch(table, i, 0, j)
                    - Real_mul_float(&wrong_ratio, base_factor) < 0.00000001,
                    "Scale_get_pitch() didn't use the retuned ratios.");
            fail_unless(Scale_get_pitch(table, i, 0, j)
                    - Real_mul_float(&note_ratio, base_factor) < 0.00000001,
                    "Scale_get_pitch() didn't calculate the pitch accurately enough.");
            Real_mul(&note_ratio, &note_ratio, &oct_mul);
            Real_mul(&wrong_ratio, &wrong_ratio, &oct_mul);
...
{
    Scale_get_pitch(NULL, 0, -1, KQT_SCALE_MIDDLE_OCTAVE);
}
END_TEST
...
START_TEST (get_pitch_break_index1)
{
    ...
    Scale_get_pitch(table, -1, -1, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_index2)
{
    ...
    Scale_get_pitch(table, INT_MIN, -1, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_index3)
{
    ...
    Scale_get_pitch(table, KQT_SCALE_NOTES, -1, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_index4)
{
    ...
    Scale_get_pitch(table, INT_MAX, -1, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_mod1)
{
    ...
    Scale_get_pitch(table, 0, KQT_SCALE_NOTE_MODS, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_mod2)
{
    ...
    Scale_get_pitch(table, 0, INT_MAX, KQT_SCALE_MIDDLE_OCTAVE);
...
START_TEST (get_pitch_break_octave1)
{
    ...
    Scale_get_pitch(table, 0, -1, KQT_SCALE_OCTAVE_FIRST - 1);
...
START_TEST (get_pitch_break_octave2)
{
    ...
    Scale_get_pitch(table, 0, -1, INT_MIN);
...
START_TEST (get_pitch_break_octave3)
{
    ...
    Scale_get_pitch(table, 0, -1, KQT_SCALE_OCTAVE_LAST + 1);
...
START_TEST (get_pitch_break_octave4)
{
    ...
    Scale_get_pitch(table, 0, -1, INT_MAX);
...
START_TEST (retune)
{
    ...
    {
        Real_init_as_frac(&note_ratio, nums[i], dens[i]);
        Real_copy(&calculated, Scale_get_note_ratio(table, i));
        fail_unless(Real_cmp(&calculated, &note_ratio) == 0,
                "Scale_retune() incorrectly modified the original ratio"
                " from %d/%d to %lld/%lld.",
                nums[i], dens[i],
    ...
    for (i = 0; i < 12; ++i)
    {
        ...
        fail_unless(Real_cmp(Scale_get_note_ratio(table, i), &note_ratio) == 0,
                ...
                " from %d/%d to %lld/%lld.",
                nums[i], dens[i],
                Real_get_numerator(Scale_get_note_ratio(table, i)),
                Real_get_denominator(Scale_get_note_ratio(table, i)));
        fail_unless(Real_cmp(&note_ratio, &table->notes[i].ratio_retuned) == 0,
                "Scale_retune() reset note incorrectly"
    ...
    for (i = 0; i < 12; ++i)
    {
        fail_unless(Real_cmp(Scale_get_note_ratio(table, i), &(table->notes[i].ratio_retuned)) == 0,
                "Scale_retune() didn't reset correctly.");
    }
    ...
    {
        int new_base = (7 * i) % 12;
        Scale_retune(table, new_base, new_base);
    }
    ...
    for (i = 0; i < 12; ++i)
    {
        ...
        fail_unless(Real_cmp(&note_ratio, &calculated) == 0,
                "Scale_retune() retuned note to %lld/%lld"
                " instead of %lld/%lld.",
                Real_get_numerator(&calculated),
                Real_get_denominator(&calculated),
    ...
    Scale_retune(table, 0, 0);

    for (i = 0; i < 12; ++i)
    {
        fail_unless(Real_cmp(Scale_get_note_ratio(table, i), &(table->notes[i].ratio_retuned)) == 0,
                "Scale_retune() didn't handle trivial retuning correctly.");
    }

    Scale_retune(table, 16, 0);
    ...
    for (k = 1; k < KQT_SCALE_NOTES; ++k)
    {
        ...
        new_ref = k;
        Scale_retune(table, new_ref, fixed_point);
        ...
        Real_div(&phafix_to_old_ratio,
                Scale_get_note_ratio(table, (table->note_count + fixed_point - k) % table->note_count),
                Scale_get_note_ratio(table, 0));
        fail_unless(Real_cmp(&fix_to_new_ratio, &phafix_to_old_ratio) == 0,
                "Scale_retune() didn't calculate new_ref correctly.");
                ...
                ++i, new_ref = (new_ref + 1) % KQT_SCALE_NOTES)
        {
            ...
            Real_div(&orig, Scale_get_note_ratio(table, i), Scale_get_note_ratio(table, i - 1));
...
START_TEST (retune_break_new_ref1)
{
    ...
    Scale_retune(table, KQT_SCALE_NOTES, 0);
...
START_TEST (retune_break_fixed_point3)
{
    ...
    Scale_retune(table, 0, KQT_SCALE_NOTES);
...
START_TEST (drift)
{
    ...
    Scale_set_note(table, 1, &note_ratio);
    Real_init_as_frac(&note_ratio, 9, 8);
    Scale_set_note(table, 2, &note_ratio);
    Real_init_as_frac(&note_ratio, 6, 5);
    Scale_set_note(table, 3, &note_ratio);
    Real_init_as_frac(&note_ratio, 5, 4);
    Scale_set_note(table, 4, &note_ratio);
    Real_init_as_frac(&note_ratio, 4, 3);
    Scale_set_note(table, 5, &note_ratio);
    Real_init_as_frac(&note_ratio, 45, 32);
    Scale_set_note(table, 6, &note_ratio);
    Real_init_as_frac(&note_ratio, 3, 2);
    Scale_set_note(table, 7, &note_ratio);
    Real_init_as_frac(&note_ratio, 8, 5);
    Scale_set_note(table, 8, &note_ratio);
    Real_init_as_frac(&note_ratio, 5, 3);
    Scale_set_note(table, 9, &note_ratio);
    Real_init_as_frac(&note_ratio, 9, 5);
    Scale_set_note(table, 10, &note_ratio);
    Real_init_as_frac(&note_ratio, 15, 8);
    Scale_set_note(table, 11, &note_ratio);

    Real calculated;
    Scale_retune(table, 2, 2); // II -- 
    Real_init_as_frac(&note_ratio, 27, 20);
    Real_copy(&calculated, &(table->notes[5].ratio_retuned));
    fail_unless(Real_cmp(&note_ratio, &calculated) == 0,
            "Scale_retune() didn't retune correctly.");

    Scale_retune(table, 5, 5); // IV -- 
    Real_init_as_frac(&note_ratio, 243, 160);
    Real_copy(&calculated, &(table->notes[7].ratio_retuned));
    fail_unless(Real_cmp(&note_ratio, &calculated) == 0,
            "Scale_retune() didn't retune correctly.");

    Scale_retune(table, 7, 0); // V -- C retained for sustain
    Real_init_as_frac(&note_ratio, 81, 80);
    Real_copy(&calculated, &(table->notes[0].ratio_retuned));
    fail_unless(Real_cmp(&note_ratio, &calculated) == 0,
            "Scale_retune() didn't retune correctly.");

    Scale_retune(table, 0, 7); // I -- C: 81/80 -> drift by a syntonic comma
    Real_init_as_frac(&note_ratio, 81, 80);
    Real_copy(&calculated, &(table->notes[0].ratio_retuned));
    fail_unless(Real_cmp(&note_ratio, &calculated) == 0,
            "Scale_retune() didn't retune correctly.");

    Scale_drift(table, &note_ratio);

    int64_t num = Real_get_numerator(&note_ratio);
    int64_t den = Real_get_denominator(&note_ratio);
    fail_unless(num == 81 && den == 80,
            "Scale_drift() returned %lld/%lld instead of 81/80.",
